...
from torch.utils.data import DataLoader as uDataLoader
...
class Graph_Deepfm(nn.Module):

    def __init__(self, feat_sizes,ratio,new_features_length,num_features, sparse_feature_columns, dense_feature_columns,dnn_hidden_units=[400, 400,400], dnn_dropout=0.0, ebedding_size=4,
                 l2_reg_linear=0, l2_reg_embedding=0.00001, l2_reg_dnn=0, edge_index=None,init_std=0.0001, seed=1024,
                 device='cpu'):
        ...
        # Initialize the convolutional layer and pooling layer
        self.conv1 = GATConv(1, 1, add_self_loops=True,dropout=0.9).to(device)
        self.lin1 = Linear(1,1).to(device)
        self.conv2 = GATConv(1, 1, add_self_loops=True,dropout=0.9).to(device)
        self.lin2 = Linear(1, 1).to(device)
        self.conv3 =GCNConv(1,1 ).to(device)
        self.pool1 = SAGPooling(1,ratio=self.ratio).to(device)
        # self.feature_index establishes a mapping from features to column names to the relative positions in the input data X
    ...
    def forward(self, X):
        ...
        dense_emb = X[:,len(self.sparse_feature_columns):]
        ...
        for j in range(conv_featrues.shape[0]):
            ...
            data = Data(x=datax, edge_index=self.edge_index)
            dataset.append(data)
    ...
    def fit(self, train_input, y_label, val_input, y_val, batch_size=1024, epochs=15, verbose=50):
        self.batch_size =batch_size
        x = [train_input[feature] for feature in self.feature_index]

        for i in range(len(x)):
            if len(x[i].shape) == 1:
                x[i] = np.expand_dims(x[i], axis=1)  # Expand into 2 dimensions for subsequent concatenation

        train_tensor_data = uData.TensorDataset(torch.from_numpy(np.concatenate(x, axis=-1)), torch.from_numpy(y_label))
        train_loader = uDataLoader(dataset=train_tensor_data,shuffle=True ,batch_size=batch_size)

        print(self.device, end="\n")
        model = self.train()
        loss_func = F.binary_cross_entropy
        optimizer = optim.Adam(model.parameters(), lr=0.0012, weight_decay = 0.0)
        ...
        for epoch in range(epochs):
            ...
            if (epoch % verbose == 0):
                ...
                train_logloss.append(self.val_auc_logloss(val_input, y_val, batch_size=self.batch_size))
                print(" ")
    ...
    def predict(self, test_input, batch_size = 256, use_double=False):
        ...
        tensor_data = uData.TensorDataset(
            torch.from_numpy(np.concatenate(x, axis=-1)))
        test_loader = uDataLoader(
    ...
    def val_auc_logloss(self, val_input, y_val, batch_size=50000, use_double=False):
        ...
        print(round(log_loss(y_val, pred_ans), 4),
              round(roc_auc_score(y_val, pred_ans), 4),
              round(accuracy_score(y_val, pred), 4),
              round(metrics.f1_score(y_val, pred), 4),
              round(metrics.recall_score(y_val, pred), 4),
              round(metrics.precision_score(y_val, pred), 4)
              )
        return(round(log_loss(y_val, pred_ans), 4))
